...
public class CrearPuzzle : MonoBehaviour
{
    ...
    void Start()
    {
		iniciarMinijuego(15, 13, 7, 1, 10.0f);
	...
	**/
	void GenerarSlots(int ancho, int alto, int filaPuntoInicio, int filaPuntoFinal, float delayFlujoTuberia){
		...
		StartCoroutine(matrizSlots[0,filaPuntoInicio-1].transform.GetChild(0).GetComponent<TipoTuberia>().ActivarTuberia(matrizSlots, 0, filaPuntoInicio-1, 1, 0));
	}
